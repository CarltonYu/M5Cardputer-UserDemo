...
| **MPU6050 IMU** | I2C SDA | GPIO 17 | 与内部 I2C 共享 |
| | I2C SCL | GPIO 18 | 与内部 I2C 共享 |
| | AD0 | GND | 地址 0x68（接 VCC 则 0x69） |
| | INT | 不接 | 可选接 GPIO 21，当前未使用 |
...
## MPU6050 IMU 模块使用注意事项

1. **模块型号**：GY-521（MPU6050 转接板），或任何 MPU6050 模块。
2. **供电**：3.3V，与 ESP32 共用。模块自带 LDO 也可接 5V。
3. **I2C 地址**：AD0 接 GND = `0x68`，AD0 接 VCC = `0x69`。本项目默认 `0x68`。
4. **I2C 总线**：与 ES8311、TCA8418 共享内部 I2C（GPIO 17/18）。
5. **SDA/SCL 线长**：建议不超过 5 cm，远离电源线和天线。
6. **数据范围**：加速度 ±2g，陀螺仪 ±250 dps，温度 ±1°C 精度。

### 接线图

```
MPU6050 (GY-521)          ESP32-S3-N16R8
┌──────────────┐          ┌──────────────┐
│ VCC          │──────→   │ 3.3V         │
│ GND          │──────→   │ GND          │
│ SCL          │──────→   │ GPIO 18      │
...
| 2026-06-16 | 增加 MPU6050 IMU 模块引脚、接线图及使用注意事项 | - |
